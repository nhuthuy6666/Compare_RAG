...
# Render bảng riêng cho phương pháp Retrieval + Answer Quality Evaluation.
def render_retrieval_answer_quality_table(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "## Retrieval + Answer Quality Evaluation",
        "",
        "| Rank | System | Profile | Overall | Answer | Retrieval | Faithfulness | MRR | Semantic |",
        "| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for rank, row in enumerate(rows, start=1):
        lines.append(
            "| {rank} | {system} | {profile} | {overall:.4f} | {answer:.4f} | {retrieval:.4f} | {faithfulness:.4f} | {mrr:.4f} | {semantic:.4f} |".format(
                rank=rank,
                system=row["system"],
                profile=row.get("profile_name", "default"),
                overall=to_float(row, "overall_score"),
...
# Render bảng riêng cho phương pháp System Performance Evaluation.
def render_system_performance_table(rows: list[dict[str, str]]) -> list[str]:
    sorted_rows = sorted(
        rows,
        key=lambda row: (
            to_float(row, "latency_budget_violation_rate"),
            int(row.get("errors", "0") or 0),
            to_float(row, "latency_ms"),
        ),
    )
    lines = [
        "## System Performance Evaluation",
        "",
        "| Rank | System | Profile | Latency (ms) | Budget Violation | Errors |",
        "| --- | --- | --- | ---: | ---: | ---: |",
    ]
    for rank, row in enumerate(sorted_rows, start=1):
        lines.append(
            "| {rank} | {system} | {profile} | {latency:.2f} | {budget_violation:.4f} | {errors} |".format(
                rank=rank,
                system=row["system"],
                profile=row.get("profile_name", "default"),
                latency=to_float(row, "latency_ms"),
...
# Ghép báo cáo chỉ tập trung vào Retrieval + Answer Quality.
def build_retrieval_answer_quality_report(
    rows: list[dict[str, str]],
    strength_rows: list[dict[str, str]] | None = None,
) -> str:
    sorted_rows = sorted(rows, key=lambda row: to_float(row, "overall_score"), reverse=True)
    lines = render_metadata(sorted_rows)
    lines.extend(render_retrieval_answer_quality_table(sorted_rows))
    lines.extend(render_strength_tables(strength_rows or []))
    return "\n".join(lines)


# Ghép báo cáo chỉ tập trung vào System Performance.
def build_system_performance_report(rows: list[dict[str, str]]) -> str:
    sorted_rows = sorted(rows, key=lambda row: to_float(row, "overall_score"), reverse=True)
    lines = render_metadata(sorted_rows)
    lines.extend(render_system_performance_table(sorted_rows))
    return "\n".join(lines)
